	...
	public static class StringAlgorithm
	{
		...
		public static int[] ZAlgorithm<T>(ReadOnlySpan<T> s)
		{
			...
			for (int i = 1, j = 0; i < n; ++i) {
				...
				while (i + k < n && EqualityComparer<T>.Default.Equals(s[k], s[i + k])) {
					++k;
				}
		...
		//original: ttps://github.com/key-moon/ac-library-cs
		public static int[] LcpArray(string s, int[] saffixArray) => LcpArray(s.AsSpan(), saffixArray);
		public static int[] LcpArray<T>(T[] s, int[] saffixArray) => LcpArray((ReadOnlySpan<T>)s, saffixArray);
		public static int[] LcpArray<T>(ReadOnlySpan<T> s, int[] saffixArray)
		{
